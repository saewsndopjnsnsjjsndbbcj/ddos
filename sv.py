#!/usr/bin/env python3
# sv.py -- SAFE simulation for Termux / Render (no network)
# Usage:
#   python3 sv.py [duration_seconds] [tick_ms] [summary_every]
# Example:
#   python3 sv.py 60 100 10

import sys, time, random, signal

# ====== Tham số mặc định ======
try:
    duration = int(sys.argv[1]) if len(sys.argv) > 1 else 30      # thời gian chạy (giây)
    tick_ms = int(sys.argv[2]) if len(sys.argv) > 2 else 100      # khoảng in ra (ms)
    summary_every = int(sys.argv[3]) if len(sys.argv) > 3 else 10 # số tick in tóm tắt
except ValueError:
    print("❌ Sai cú pháp! Dùng: python3 sv.py [duration] [tick_ms] [summary_every]")
    sys.exit(1)
...
def step_counts():
    global good, bad
    bad += random.randint(0, 149)  # Bad tăng nhanh hơn
    good += random.randint(0, 2)   # Good tăng ít

def handle_sigint(signum, frame):
    global running
    running = False
    print("\n[SAFE-SIM] ⚠️ Dừng lại (Ctrl+C). Đang thoát an toàn...")
    print(f"Final counts: ticks={count} Good={good} Bad={bad}")
    sys.exit(0)

# ====== Bắt Ctrl+C ======
signal.signal(signal.SIGINT, handle_sigint)

print(f"[SAFE-SIM] ✅ Bắt đầu mô phỏng trong {duration}s (tick {tick_ms}ms). Nhấn CTRL+C để dừng.")
...
while running and (time.time() - start_time) < duration:
    ...
    # Số dòng in mỗi tick (muốn spam nhiều thì tăng số này)
    repeats = 1
    for _ in range(repeats):
        print("the service is unavailable.")

    # In tóm tắt mỗi N tick
    if count % summary_every == 0:
        print(f"ended count: {count} Good: {good} Bad: {bad}")

    time.sleep(interval)

print(f"[SAFE-SIM] 🏁 Kết thúc. Tổng tick: {count}. Good: {good}. Bad: {bad}.")
const UAs = [
