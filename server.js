...
// Thay thế đoạn if (process.argv.length <= 2) { ... process.exit(-1); }
// bằng đoạn này. Mục đích: KHÔNG chạy hành vi tấn công.
// Chỉ cho phép targets trong allowlist (ví dụ: localhost, 127.0.0.1, private IP dev).
const os = require('os');
const url = require('url');

function safeParseArgs(allowlist) {
  const usage = () => {
    console.error('USAGE (safe mode): node server.js <url> <time_seconds>');
  ...
  if (process.argv.length <= 2) {
    usage();
    // don't crash with strange exit codes; exit gracefully
    process.exit(1);
  }
...
const ALLOWLIST = [
  // add dev machine hostnames or internal test server host/IP here, if needed
