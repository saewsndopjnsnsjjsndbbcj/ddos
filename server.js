// --- SAFE ARG PARSER & GUARD (Render + Termux safe mode) ---
const fs = require('fs');
...
function safeParseArgs(allowlist) {
  ...
  // ✅ Hỗ trợ mặc định cho Render/Termux (nếu không có args)
  let target = process.argv[2] || process.env.TARGET_URL || 'http://localhost';
  let time = parseInt(process.argv[3], 10) || parseInt(process.env.TIME, 10) || 10;
  ...
  try {
    ...
    // chỉ cho phép target trong allowlist hoặc localhost
    const isLocal = ['localhost', '127.0.0.1', '::1'].includes(host);
    const isAllowed = isLocal || allowlist.includes(host);

    if (!isAllowed) {
      console.error('[SAFE MODE] ❌ Target not allowed:', host);
      console.error('This runtime prevents outward network tests.');
      usage();
      process.exit(1);
    }

    return { target, host, time };
  } catch (err) {
...
// ✅ Cho phép chỉ IP nội bộ (local network)
const ALLOWLIST = [
  '192.168.1.100', // ví dụ LAN
  '10.0.2.2'
];
...
// ví dụ chạy test
console.log(`[RUNNING SAFE TEST] Connecting to ${host} for ${time}s...`);

setTimeout(() => {
  console.log(`[SAFE DONE] Test finished for ${target}`);
  process.exit(0);
}, time * 1000);
const UAs = [
