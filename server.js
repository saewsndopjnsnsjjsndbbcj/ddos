// ------------------ SAFE HEADER (paste here) ------------------
// Ghi chú: Đoạn này bảo vệ khỏi chạy nhầm với target ngoài allowlist.
// Sao lưu file trước khi dán: cp server.js server.js.bak
...
// --- SAFE ARG PARSER & GUARD ---
// Chỉ cho phép targets trong ALLOWLIST hoặc localhost variants.
// Nếu không có args hoặc target không được phép => exit(1) (không crash -255).
function safeParseArgs(allowlist) {
  const usage = () => {
    console.error('USAGE (safe mode): node server.js <target_url> <time_seconds>');
    console.error('Example (local test): node server.js http://localhost 10');
    console.error('Allowed targets:', allowlist.join(', '));
  };
  ...
  let time = parseInt(process.argv[3], 10) || 10;
  ...
  try {
    ...
    // allow only if host in allowlist (exact match) or is localhost variants
    const isLocal = ['localhost', '127.0.0.1', '::1'].includes(host);
    const isAllowed = isLocal || allowlist.includes(host);

    if (!isAllowed) {
      console.error('Target not allowed in safe mode:', host);
      console.error('This runtime is configured to prevent outward network tests.');
      usage();
      process.exit(1);
    }

    // return parsed safe args
    return {
      target: target,
      host: host,
      time: time
    ...
    console.error('Error parsing target URL:', err && err.message);
    usage();
    process.exit(1);
  }
}

// Customize allowlist for your dev environment (only private/dev hosts)
// NOTE: localhost and 127.0.0.1 are already allowed by default.
const ALLOWLIST = [
  '192.168.1.100', // add your dev IP(s) here if needed
  '10.0.2.2'       // example
];

const SAFE_ARGS = safeParseArgs(ALLOWLIST);

// export or set variables for the rest of your file
const target = SAFE_ARGS.target;
const host = SAFE_ARGS.host;
const time = SAFE_ARGS.time;

console.log('[SAFE MODE] target=', target, 'host=', host, 'time(s)=', time);

// Global exception handlers (keeps readable logs)
process.on('uncaughtException', function (e) {
    console.warn('Uncaught exception:', e && e.stack ? e.stack : e);
...
// ------------------ END SAFE HEADER ------------------
// After this line you can place the rest of your code that uses `target`, `host`, `time`.
// IMPORTANT: Do NOT re-declare `const url = require('url');` or redeclare variables already used above.
const UAs = [
	...
	"WebCopier v4.6",
	"Web Downloader/6.9",
	"WebZIP/3.5 (https://phanmemsunwin.fun)",
	"Wget/1.9.1",
